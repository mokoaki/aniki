...
describe FileObject do
  # before { @file_object = FactoryGirl.create(:file_object) }

  let(:root_object)       { FactoryGirl.create(:root_object) }
  let(:trash_object)      { FactoryGirl.create(:trash_object) }
  let(:directory_object)  { FactoryGirl.create(:directory_object) }
  let(:file_object)       { FactoryGirl.create(:file_object) }
  let(:trash_file_object) { FactoryGirl.create(:trash_file_object) }
  ...
  describe 'relations' do
    it 'parent_directory' do
      root_object #これが実行されないとparent_directory実行時にroot_objectが存在しない もっとうまいやり方が絶対ある
      expect(file_object.parent_directory).to eq(root_object)
    end

    it 'children' do
      directory_object  #これが実行されないと(ry
      file_object

      expect(root_object.children).to eq([directory_object, file_object])
    end
  end
  ...
  describe 'public methods' do
    it 'file_save' do
      root_object
      file_object = root_object.children.new
      file_object.file_save(nil, 'zy')
